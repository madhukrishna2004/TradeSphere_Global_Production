...
#from data import BLOGS, TESTIMONIALS
...
SMTP_PASSWORD = os.getenv("SMTP_PASSWORD", "your-app-password-here")  # ⚠️ must be set in .env

# ----------------- LOGIN -----------------
@app.route("/login", methods=["GET", "POST"])
def login():
    if request.method == "POST":
        identifier = request.form.get("username")  # email or phone
        ...
        try:
            ...
            if user:
                ...
                print("⚠️ No user found with given identifier")

            if user and check_password_hash(user[4], password):
                session["user"] = user[2]  # store email in session
...
def forgot_password():
    if request.method == "POST":
        try:
            ...
            cur.execute("SELECT id FROM users_main WHERE email = %s", (email,))
            user = cur.fetchone()
            ...
            print(f"✅ Email found for user_id: {user[0]}")
            ...
            conn.commit()
            print(f"✅ Reset token stored for user {user[0]} (expires {expires_at})")
            ...
            # Return JSON for AJAX / Render template for form
            if request.is_json:
...
def reset_password(token):
    ...
    conn = get_db_connection()
    # FIXED: Use DictCursor instead of dictionary=True
    ...
    try:
        ...
        print(f"✅ Token valid for user_id {entry['user_id']}")
        ...
        if request.method == "POST":
            ...
            # Update password
            cur.execute("UPDATE users_main SET password = %s WHERE id = %s", (hashed_pw, entry["user_id"]))
...
def send_reset_email(to_email, reset_link):
    ...
    msg = MIMEText(body, "html")  # send as HTML email
    msg["Subject"] = subject
    msg["From"] = f"TradeSphere Global <{SMTP_USERNAME}>"
    msg["To"] = to_email
...
# ------------------ Mail Config (Zoho SMTP) ------------------
app.config['MAIL_SERVER'] = 'smtp.zoho.in'
app.config['MAIL_PORT'] = 587
app.config['MAIL_USE_TLS'] = True
app.config['MAIL_USERNAME'] = os.getenv("SMTP_USERNAME")    
app.config['MAIL_PASSWORD'] = os.getenv("SMTP_PASSWORD")
app.config['MAIL_DEFAULT_SENDER'] = os.getenv("SMTP_SENDER")   
...
def contact():
    if request.method == "POST":
        ...
        # Send Email to Admin
        try:
            admin_email = os.getenv("ADMIN_EMAIL", "connect@trade-sphereglobal.com")
            msg = Message(
